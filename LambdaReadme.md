...
> Implementing Financial AWS Lambda function in Visual Studio. 
> Uses **Docker** to run functions in AWS Amplify.
> Code commited to GitHub, then auto-deployed to AWS.
...
These base images contain: 
* Amazon Linux Base operating system, 
* Runtime for a given language, dependencies 
* Lambda Runtime Interface Client (RIC), which implements the Lambda Runtime API. 

Sole 'payload' is my AWS Lambda function **FV** that will be exposed through Amazon API Gateway as a HTTP *Get* operation. 
...
---
# FV Method - NOTE: Documentation will be improved when project done spring 2025!
